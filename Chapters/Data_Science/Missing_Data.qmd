...
Imagine a trial where a subject it measured each week. The missing data pattern is considered monotone, if there are not any missing values between two observed data points. For example, let's say the trial gives provides values $Y_1,Y_2,\dots,Y_52$, where the index is the week. If we have $Y_1$ and $Y_3$, but not $Y_2$ then the data pattern is not monotone.

### Non-monotone missingness
When there is a missing value between two observed data points. For example, let's say the trial gives provides values $Y_1,Y_2,\dots,Y_52$, where the index is the week. If we have $Y_1$ and $Y_3$, but not $Y_2$ then the data pattern is non-monotone.
