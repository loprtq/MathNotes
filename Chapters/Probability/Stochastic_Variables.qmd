...
{{< include /Examples/Probability/Coin_Toss.qmd >}}
...
F_X(x) = \Pr{X \leq x}.
$$
The phrasing of the CDF might sound familiar if you have previously worked with p-values (see @sec-p-values). Similarly, if you have heard of quantiles or median, these are all related to the CDF. Quantiles are the values of $x$ where $F_X(x) = q$, where $q$ is the quantile. For example, the 0.25 (25%) quantile is the value of $x$ where $F_X(x) = 0.25$. The median is a special case of the quantiles, where $q = 0.5$ (50%). The median of a stochastic variable is the value of $x$ where there is a 50% chance that the stochastic variable is less than or equal to $x$ - it is the value that splits the distribution in half.
...
$$
where $F_X$ is the CDF and $\int_0^\infty f_X(x)\,\mathrm{d}x = 1$ - the area under the curve is exactly 1, meaning that the total probability is 1. Intuitively, the PDF describes the rate of change of the CDF with respect to $x$.
...
The conditional expected value of a stochastic variable $X$ given another stochastic variable $Y$ is the expected value of $X$ given that $Y$ takes on a specific value ($y$). The conditional expected value is defined as
$$
\begin{aligned}
...
::: {.callout-tip collapse = true title = "Variance Properties" #tip-Variance_Properties}
For the stochastic variables $X$ and $Y$, and constants $a$ and $b$, the following properties hold
$$
\begin{aligned}
    \Var{a} &= 0 \\
...
The standard deviation (SD) of the stochastic variable is $\sigma = \sqrt{\Var{X}}$. The standard deviation can be thought of as what the "average" deviation from the mean would be if the units were the same as the stochastic variable, instead of being in the square of the units. For example, if we have a stochastic variable that measures height in centimeters, then the variance would be in square centimeters, while the standard deviation would be in centimeters.
...
::: {.callout-tip collapse = true title = "Covariance Properties" #tip-Covariance_Properties}
$$
\begin{aligned}
    \Cov{X, X} &= \Var{X} \\
    \Cov{X, Y} &= \Cov{Y, X} \\
