...
This part will dive deeper into some of the models used in statistics, which do not naturally fit into one of the other chapters. Additionally, it will cover some of the more general concepts related to statistical modeling.


## Prediction {#sec-Prediction}
One of the main goals of statistical modeling is to make predictions about the response variable based on the covariates. Prediction can be used for various purposes, such as forecasting future outcomes (e.g. electricity or stock prices), estimating missing values, or evaluating the performance of a model. The process of prediction involves using the fitted model to generate predicted values for new observations or for the same observations used in model fitting. The accuracy of predictions can be assessed using various metrics, such as mean squared error (MSE), mean absolute error (MAE), or R-squared. Additionally, prediction intervals can be constructed to provide a range of plausible values for the response variable, accounting for uncertainty in the model estimates and the inherent variability in the data. The ability to make accurate predictions is a key aspect of the utility of statistical models in both research and applied settings.


## Inference {#sec-Inference}
Inference in statistical modeling involves drawing conclusions about the population parameters based on the observed data - utilising the fitted model to understand the underlying relationships between variables. This includes estimating the parameters, testing hypotheses, and constructing confidence intervals. Inference allows researchers to make probabilistic statements about the relationships between variables and the effects of interventions, providing a basis for decision-making and scientific understanding.

### Confidence Intervals {#sec-Confidence_Intervals}
A confidence interval is a range of values that is estimated to contain the true parameter with a specified level of confidence. Rather than providing a single point estimate of a parameter, a confidence interval presents the uncertainty in the estimation. Confidence intervals are constructed around a point estimate using the sampling distribution of that estimate. The width of the confidence interval reflects the precision of the estimate, with narrower intervals indicating more precise estimates. The level of confidence, typically expressed as a percentage (e.g., 95%), indicates the degree of certainty that the interval contains the true parameter value - a 95% confidence interval does not mean there is a 95% probability that the true parameter lies within the interval. Rather, it means that if the data collection (e.g. a clinical trial) and interval construction process were repeated indefinitely, approximately 95% of the constructed intervals would contain the true parameter value.
...
### Continuous Variables {#sec-Continuous_Variables}
...
### Confounding Covariates {#sec-Confounding_Covariates}
Confounding covariates are variables that are related to both the response variable and one or more of the covariates. They can introduce bias in the estimation of the effect of the covariateson the response variable if not properly accounted for. Identifying and adjusting for confounding covariates is crucial for obtaining valid inferences in statistical models.


## Interaction Terms {#sec-Interaction_Terms}
Interaction terms are used when the effect of one covariate on the response variable depends on the level of another covariate. In other words, the association is not additive - the combined effect of two covariates is different from the sum of their separate effects. As an example, in a linear regression model (see @sec-Linear_Regression) with treatment and age, an interaction term between treatment and age allows the treatment effect to vary with age rather than assuming a constant treatment effect across all ages.

Adding interaction terms increases model complexity and can reduce interpretability, hence they should ideally be pre-specified or strongly justified by domain knowledge. When interaction terms are included, interpretation should focus on conditional effects (effect of one covariate at specific values or levels of the other) rather than on main effects alone.
...
{{< include /Figures/Statistical_Models/Count_Data.qmd >}}

Count data often exhibit characteristics that violate the assumptions of linear regression (see @sec-Linear_Regression). The distribution of counts is typically right-skewed (e.g. @fig-Count_Data), particularly when the mean count is small, and the variance often increases with the mean. Additionally, the discrete nature of count data means that predictions from linear regression could inappropriately produce negative values. For these reasons, specialized models such as Poisson regression (see @sec-Poisson_Regression) and negative binomial regression (see @sec-Negative_Binomial_Regression) are used to model count data. These models ensure that predicted values are non-negative and properly account for the distributional characteristics of count outcomes.

### Zero-Inflated Data {#sec-Zero_Inflated_Data}
Zero-inflated data arise when count data contains an excess of zero values - beyond what would be expected under standard count distributions. An excess of zeros can occur if something is preventing an event from occuring. For example, if you are counting how many fish are caught in a lake, and the lake is has no fish, then you will always catch zero fish. However, if the lake has fish, there is still a chance to catch zero fish due to factors such as skill or luck.

Zero-inflated models address this by combining two components: a binary component that models the probability of being in the "zero state" and a count component that models the distribution of positive counts. Zero-inflated Poisson regression (see @sec-Zero_Inflated_Poisson_Regression) and zero-inflated negative binomial regression (see @sec-Zero_Inflated_Negative_Binomial_Regression) are commonly used approaches. These models allow the analyst to investigate both the factors that influence whether an individual falls into the zero-inflation process and the factors that influence the rate of events among those not in the zero state. By explicitly modeling the excess zeros, zero-inflated models provide better fit and more valid inferences compared to standard count models when zero-inflation is present.

### Incidence Rates {#sec-Incidence_Rates}
...
## Model Selection Criteria {#sec-Model_Selection_Criteria}
...
### Akaike Information Criterion {#sec-AIC}
The AIC is based on information theory and estimates the relative quality of statistical models. For a given dataset, AIC estimates the information lost when a particular model is used to approximate the true data-generating process. The model with the lowest AIC is preferred as it represents the best approximation with the least information loss. AIC is defined as:
$$
\text{AIC} = -2\log{\mathcal{L}} + 2k
$$
where $\mathcal{L}$ is the maximized likelihood of the model and $k$ is the number of estimated parameters. The first term measures how well the model fits the data (lower values indicate better fit), while the second term penalizes model complexity. For example, comparing two linear regression models where one includes an additional predictor variable, AIC helps determine whether the improved fit from the extra predictor justifies the added complexity.

### Bayesian Information Criterion {#sec-BIC}
The BIC is derived from Bayesian model selection principles - it approximates the posterior probability of a model being the true model given the data. Unlike AIC, which focuses on predictive accuracy, BIC tends to favor simpler models more strongly, especially as sample size increases. BIC is defined as:
$$
\text{BIC} = -2\log{\mathcal{L}} + k\log{n}
$$
where $n$ is the sample size, $\mathcal{L}$ is the maximized likelihood of the model, and $k$ is the number of estimated parameters. The penalty term $k\log{n}$ grows with sample size, making BIC increasingly conservative as more data becomes available.
