...
Specialised statistical methods are required to properly analyse repeated measures data. Linear mixed models (see @sec-Linear_Mixed_Model) are particularly well-suited for this purpose as they can account for both fixed effects and random effects. The choice of covariance structure (see @sec-Covariance_Structures) is crucial for these models, as different structures make different assumptions about how observations within an individual are correlated.
