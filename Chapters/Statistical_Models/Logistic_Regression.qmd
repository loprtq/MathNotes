...
The odds ratio (OR) is often utilized when there is a desire to compare the odds of an outcome in one group to the odds in another. The odds of an event with probability $p$ is $\frac{p}{1 - p}$, and the OR is the ratio of odds between two groups:
$$
\text{OR} = \frac{\frac{p_1}{1 - p_1}}{\frac{p_0}{1 - p_0}}
$$
In logistic regression, the OR for covariate $x_j$ is obtained by exponentiating its coefficient: $\text{OR}_j = \exp{\alpha_j}$.

Non-collapsible. Not a causal estimand.

- $\text{OR} < 1$: decreased odds of the outcome, compared to reference

- $\text{OR} = 1$: No difference in odds, compared to reference

- $\text{OR} > 1$: increased odds of the outcome, compared to reference

::: {#rem-OR}
The confidence intervals for ORs are only symmetrical on the log-scale, and not the usually presented exp-scale.
:::