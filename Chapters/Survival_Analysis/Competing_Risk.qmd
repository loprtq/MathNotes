...
The CIF (See @eq-CIF) for a specific event, $F_j(t) = \operatorname{P}(T\leq t, C = j) = \int_t^t h_j(u) \cdot S(u)\,\textrm{d}u$, describes. Similar to the overall hazard ratio, the overall CIF can be described by $F(t) = \sum_{j=1}^k F_j(t)$.
